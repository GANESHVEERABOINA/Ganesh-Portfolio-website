...
import { 
  ...
  Hexagon, ShieldAlert, ShieldCheck, Layers,
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-black font-sans text-white selection:bg-white/30 selection:text-white overflow-x-hidden">
      ...
      <main className="w-full px-6 md:px-12 lg:px-20 py-20 space-y-32">
                   ...
                   return (
                     ...
                     >
                        <div className={`mb-4 p-3.5 rounded-xl border transition-all duration-300
                           ${isHovered ? 'text-black bg-white border-white shadow-[0_0_20px_rgba(255,255,255,0.8)]' : 'text-white bg-white/10 border-white/30'}`} 
                           style={{ transform: "translateZ(30px)" }}>
                           {node.category.includes('Cloud') ? <Cloud size={26} color={isHovered ? "black" : "white"} /> : 
                            node.category.includes('Containers') ? <Box size={26} color={isHovered ? "black" : "white"} /> : 
                            node.category.includes('CI/CD') ? <GitBranch size={26} color={isHovered ? "black" : "white"} /> : 
                            node.category.includes('IaC') ? <Code2 size={26} color={isHovered ? "black" : "white"} /> : 
                            node.category.includes('Quality') ? <ShieldCheck size={26} color={isHovered ? "black" : "white"} /> : 
                            node.category.includes('Artifacts') ? <Package size={26} color={isHovered ? "black" : "white"} /> : 
                            node.category.includes('OS') ? <Terminal size={26} color={isHovered ? "black" : "white"} /> : 
                            <Server size={26} color={isHovered ? "black" : "white"} />}
                        </div>
          ...
          <div className="lg:hidden relative w-full px-4 mx-auto mt-8">
             ...
             <div className="flex flex-col gap-6 relative z-10 w-full">
                {mySkillsData.map((node, i) => (
                   <motion.div key={`mob-node-${i}`} initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} whileTap={{ scale: 0.95 }} transition={{ delay: i * 0.1, type: "spring", stiffness: 100 }} className="relative pl-12 w-full cursor-pointer">
                      ...
                      <div className="bg-black border border-white/20 rounded-2xl p-5 shadow-lg w-full relative z-10 active:border-white transition-colors">
                         ...
                         <div className="flex flex-wrap gap-2">
                            {node.technologies.map((tech: string, j: number) => (
                               <span key={j} className="text-[11px] font-bold bg-black border border-white/30 text-white px-2.5 py-1.5 rounded-lg shadow-inner">{tech}</span>
                            ))}
                         </div>
                      </div>
                   </motion.div>
        ...
        <div className="w-full flex justify-end mt-10">
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            className="text-white text-5xl md:text-6xl antialiased drop-shadow-[0_0_10px_rgba(255,255,255,0.6)] cursor-default transition-all duration-300 hover:drop-shadow-[0_0_20px_rgba(255,255,255,1)] hover:scale-105" 
      ...
      {/* Footer / Contact */}
      <footer id="contact" className="bg-black py-12 mt-10 relative">
        ...
        <div className="w-full px-6 md:px-12 lg:px-20 text-center flex flex-col items-center">
          ...
          <p className="text-white font-medium font-mono text-sm opacity-70">© {new Date().getFullYear()} Ganesh Veeraboina. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
